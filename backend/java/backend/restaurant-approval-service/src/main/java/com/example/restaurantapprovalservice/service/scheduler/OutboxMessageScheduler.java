package com.example.restaurantapprovalservice.service.scheduler;
...
public class OutboxMessageScheduler {
    ...
    public void processOutboxMessage() {
        ...
        Optional<List<RestaurantApprovalOutboxMessageEntity>> outboxEntities = Optional.ofNullable(
            restaurantApprovalRequestOutboxRepository.findBySagaTypeAndOutboxStatus(
                    EATS_ORDER.name(),
        ...
        if (!outboxMessagesWithEntities.isEmpty()) {
            outboxMessagesWithEntities.forEach(pair -> {
                ...
                kafkaProducer.sendAndRunCallback(eatsOrderServiceConfigData.getRestaurantApprovalResponseTopicName(),
                    "key",
                    outboxMessage,
                    (metadata, exception) -> {
